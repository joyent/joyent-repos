...
/*
 * Parse a label selector string (e.g. `!foo`, `check=42`, `state=*`) into
 ...
 *      jr list -l lang=js
 *
 * By default there is an implicit `state = active` selector, unless either:
 * - `opts.allStates=true`, or
 * - `opts.labelSelectors` includes a selector for `state`.
 *
 *
...
JoyentReposManager.prototype.listRepos = function listRepos(opts, cb) {
    ...
    assert.optionalBool(opts.allStates, 'opts.allStates');
    assert.func(cb, 'cb');
    ...
    vasync.pipeline(
        {
            ...
            funcs: [
                ...
                function determineSelectors(ctx, next) {
                    ...
                    if (!opts.allStates) {
                        var stateSelectors = ctx.selectors.filter(function aSel(
                            sel
                        ) {
                            return sel.key === 'state';
                        });
                        if (stateSelectors.length === 0) {
                            log.debug(
                                'add implicit "state=active" label selector'
                            );
                            ctx.selectors.unshift(
                                _parseLabelSelector('state=active')
                            );
                        }
                    }
