 ...
 */
function parseKeyValue(kv, validKeys, options) {
    assert.string(kv, 'kv');
    ...
    var idx = kv.indexOf('=');
    if (idx === -1) {
        throw new VError('invalid key=value: "%s"', kv);
    }
    ...
    if (validKeys) {
        ...
        for (i = 0; i < validKeys.length; i++) {
            ...
            if (
                (validKey instanceof RegExp && validKey.test(k)) ||
                k === validKey
            ) {
                foundMatch = true;
                break;
    ...
    if (v === '' && options.failOnEmptyValue) {
        throw new VError('key "%s" must have a value', k);
    }
...
function editInEditor(opts, cb) {
    ...
    if (opts.filename && opts.filename.indexOf('/') !== -1) {
        ...
        tmpPath = path.resolve(
            os.tmpDir(),
            format('jr-%s-edit-%s', process.pid, opts.filename || 'text')
        );
    }
