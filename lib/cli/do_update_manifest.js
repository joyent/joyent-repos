    ...
    vasync.pipeline(
        {
            ...
            funcs: [
                ...
                function categorizeRepos(ctx, next) {
                    ...
                    var notRepoFromName = {};
                    for (repo of ctx.manifest.notRepositories) {
                        notRepoFromName[repo.name] = repo;
                    }
                    ...
                    // Entries in manifest.notRepositories that are no longer
                    // in the query results.
                    ctx.goneNotRepos = [];
                    for (repo of ctx.manifest.notRepositories) {
                        if (!candidateRepoFromName[repo.name]) {
                            ctx.goneNotRepos.push(repo);
                        }
                    }
                ...
                function removeGoneRepos(ctx, next) {
                    var repos = ctx.goneRepos.concat(ctx.goneNotRepos);
                    if (repos.length === 0) {
                        console.log(
                    ...
                    common.promptYesNo({msg: msg, default: 'y'}, function onA(
                        ...
                        if (answer !== 'y') {
                            ...
                            // for (var goneRepo of ctx.goneRepos) {
                            //    ctx.manifest.repositories = ctx.manifest.repositories.filter(
                            //        function(repo) {
                            //            return repo.name !== goneRepo.name;
                            //        }
                            //    );
                            // }

                            var goneNotRepoNames = new Set(
                                ctx.goneNotRepos.map(function aRepo(r) {
                                    return r.name;
                                })
                            );
                            ctx.manifest.notRepositories = ctx.manifest.notRepositories.filter(
                                function aRepo(r) {
                                    return !goneNotRepoNames.has(r.name);
                                }
                            );

                            // for (var goneNotRepo of ctx.goneNotRepos) {
                            //    ctx.manifest.notRepositories = ctx.manifest.notRepositories.filter(
                            //        function(repo) {
                            //            return repo.name !== goneNotRepo.name;
                            //        }
                            //    );
                            // }
                    ...
                    editReposInEditor(
                        ...
                        function onEdited(err, repos) {
                            if (err) {
                                ...
                                if (!ctx.manifest.notRepositories) {
                                    ctx.manifest.notRepositories = [];
                                }
                                for (var repo of repos) {
                                    ctx.manifest.notRepositories.push({
                                        name: repo.name
                                    });
                                }
                                tabula.sortArrayOfObjects(
