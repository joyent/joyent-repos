...
var columnsDefaultLong = [  // TODO: needed?
    'name',
    'state',
    'tags'
];
...
function do_list(subcmd, opts, args, cb) {
    ...
    var filters;
    try {
        var filters = args.map(function anArg(arg) {
            return common.parseKeyValue(arg, ['name', 'state', 'tag'], {
                typeHintFromKey: {tag: 'string'}
            });
        });
    } catch (parseErr) {
        cb(new UsageError(err, 'invalid filter arg(s)'));
        return;
    }
    ...
    vasync.pipeline(
        {
            ...
            funcs: [
                ...
                function filterThem(ctx, next) {
                    ...
                    log.debug({filters: filters}, 'filter from %d to %d repos',
                        ctx.repos.length, repos.length);
                    ctx.repos = repos;
                    next();
                },
